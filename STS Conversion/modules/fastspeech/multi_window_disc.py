...
class Discriminator2DFactory(nn.Module):
    def __init__(self, time_length, freq_length=80, kernel=(3, 3), c_in=1, hidden_size=128, sn=False, reduction='sum'):
...
class MultiWindowDiscriminator(nn.Module):
    def __init__(self, time_lengths, cond_size=0, freq_length=80, kernel=(3, 3),
                 c_in=1, hidden_size=128, sn=False, reduction='sum'):
        ...
        for time_length in time_lengths:
            conv_layer = [
                Discriminator2DFactory(
                    time_length, freq_length, kernel, c_in=c_in, hidden_size=hidden_size, sn=sn, reduction=reduction)
            ]
...
class Discriminator(nn.Module):
    def __init__(self, time_lengths=[32, 64, 128], freq_length=80, cond_size=0, kernel=(3, 3), c_in=1,
                 hidden_size=128, sn=False, reduction='sum', uncond_disc=True):
