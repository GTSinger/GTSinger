...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, cond, ret=None, ref_mels=None, mel2ph=None, infer=False):
        # cond [B, T, H]
        ...
        if not infer:
            ...
            nonpadding = (mel2ph != 0).float() if mel2ph is not None else None
            ...
            if hparams.get('pndm_speedup'):
                ...
                for i in tqdm(reversed(range(0, t, iteration_interval)), desc='sample time step',
                    ...
                    x = self.p_sample_plms(x, torch.full((b,), i, device=device, dtype=torch.long),
                                           iteration_interval, cond)
            else:
