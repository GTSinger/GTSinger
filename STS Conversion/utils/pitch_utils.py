...
def norm_f0(f0, uv, hparams, mean=None, std=None):
    mean = mean if mean is not None else hparams['f0_mean']
    std = std if std is not None else hparams['f0_std']
    is_torch = isinstance(f0, torch.Tensor)
...
def norm_interp_f0(f0, hparams, mean=None, std=None):
    is_torch = isinstance(f0, torch.Tensor)
    if is_torch:
        device = f0.device
        f0 = f0.data.cpu().numpy()
    uv = f0 == 0
    f0 = norm_f0(f0, uv, hparams, mean, std)
    if sum(uv) == len(f0):
        f0[uv] = 0
    elif sum(uv) > 0:
...
def denorm_f0(f0, uv, hparams, pitch_padding=None, min=None, max=None, mean=None, std=None):
    is_torch = isinstance(f0, torch.Tensor)
    if hparams['pitch_norm'] == 'standard':
        mean = mean if mean is not None else hparams['f0_mean']
        std = std if std is not None else hparams['f0_std']
...
def hz_to_midi(hz):
    if type(hz) == np.ndarray or type(hz) == torch.Tensor:
        non_mask = hz == 0
...
def midi_pitch_shift(midi, tgt_avg_freq):
    # print('midishape', midi.shape)
    non_mask = midi == 0
    midi_freq = midi_to_hz(midi)
    diff = midi_freq[~non_mask].mean() - tgt_avg_freq
    midi_freq[~non_mask] = midi_freq[~non_mask] - diff
    shifted_midi = hz_to_midi(midi_freq)
    shifted_midi[non_mask] = 0
    # tgt_avg_midi = hz_to_midi(tgt_avg_freq)
    # diff = int(midi[~non_mask].cpu().numpy().mean()) - tgt_avg_midi
    # midi[~non_mask] = midi[~non_mask] - int(diff)
    # shifted_midi = midi
    # shifted_midi[non_mask] = 0
    return shifted_midi.long().clamp(min=0)
