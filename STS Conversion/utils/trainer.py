...
class Trainer:
    ...
    def fit(self, task):
        if len(self.all_gpu_ids) > 1:
            mp.spawn(self.ddp_run, nprocs=self.num_gpus, args=(task,))
        else:
            self.task = task
            self.run_single_process(task)
        return 1

    def ddp_run(self, gpu_idx, task):
    ...
    def run_single_process(self, task):
        ...
        checkpoint, _ = get_last_checkpoint(self.work_dir)
    ...
    def train(self):
        ...
        while True:
            ...
            for batch_idx, batch in enumerate(train_pbar):
                ...
                should_log_metrics = (batch_idx + 1) % self.tb_log_interval == 0
    ...
    def run_training_batch(self, batch_idx, batch):
        ...
        for opt_idx, optimizer in enumerate(self.optimizers):
            ...
            if (self.global_step + 1) % self.accumulate_grad_batches == 0:
                task_ref.on_before_optimization()
                if self.amp:
    ...
    def save_checkpoint(self, epoch, logs=None):
        ...
        if current is not None and self.save_best:
            if monitor_op(current, self.best_val_results):
                best_filepath = f'{self.work_dir}/model_ckpt_best_steps_{self.global_step}.pt'
