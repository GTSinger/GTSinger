...
from singing.audio_evaluation.util import (
    gross_pitch_error, voicing_decision_error, f0_frame_error, f0_mse_error,
    f0_mae_error
)
from singing.audio_evaluation.eval_sp import load_eval_spec
...
def compute_yin(sig, sr, w_len=512, w_step=256, f0_min=80, f0_max=800,
                harmo_thresh=0.1):
    """
...
def extract_f0(samples):
    ...
    for sample in tqdm.tqdm(samples):
        ...
        yref = torchaudio.functional.resample(yref, orig_freq=sr, new_freq=24000)[:, :-2048]
        sr = 24000
...
def eval_f0_mae_error(samples):
    return eval_f0_error(samples, f0_mae_error)
...
def main(eval_f0, gpe, vde, ffe, f0rmse, show_bin):
    samples = load_eval_spec(eval_f0, None)
    if gpe or vde or ffe or f0rmse:
    ...
    if f0rmse:
        print("===== Evaluate F0 Frame Error =====")
        results = eval_f0_mae_error(f0_samples)
        print_results(results, show_bin)
