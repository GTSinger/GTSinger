...
from research.singtech.modules.zipformer.scaling import _diag
from research.singtech.modules.zipformer.scaling import *
from research.singtech.modules.zipformer.utils import is_jit_tracing, make_pad_mask
...
class ZipformerLayers(nn.Module):
    def __init__(self, cond_hidden_size= 256, encoder_dims: Tuple[int] = (256,256,256), attention_dim: Tuple[int] = (256,256,256),
                 encoder_unmasked_dims: Tuple[int] = (256,256,256), zipformer_downsampling_factors: Tuple[int] = (2,4,2),
                 nhead: Tuple[int] = (4,4,4), feedforward_dim: Tuple[int] = (256,256,256),
                 num_encoder_layers: Tuple[int] = (2,2,2), dropout: float = 0.1,
                 cnn_module_kernels: Tuple[int] = (9,9,9), pos_dim: int = 4, warmup_batches: float = 4000.0) -> None:
        super().__init__()
        ...
        self.ln = nn.Linear(encoder_dims[-1], cond_hidden_size)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        #mask = make_pad_mask(x_lens, x.size(0))
        outputs = []
...
class ZipformerEncoder(nn.Module):
    def __init__(
        self, encoder_layer: nn.Module, num_layers: int, dropout: float, warmup_begin: float, warmup_end: float) -> None:
        super().__init__()
        self.batch_count = 0
    ...
    def forward(self, src: Tensor, feature_mask: float = 1.0, mask: Optional[Tensor] = None,
                 src_key_padding_mask: Optional[Tensor] = None):
...
class RelPositionMultiheadAttention(nn.Module):
    def __init__(self, embed_dim: int, attention_dim: int, num_heads: int, pos_dim: int, dropout: float = 0.0):
        ...
        assert self.head_dim * num_heads == attention_dim, (self.head_dim, num_heads, attention_dim)
        in_proj_dim = 2 * attention_dim + attention_dim // 2 + pos_dim * num_heads
        self.in_proj = ScaledLinear(embed_dim, in_proj_dim, bias=True, initial_scale=self.head_dim**-0.25)
        self.whiten_values = Whiten(num_groups=num_heads, whitening_limit=2.0, prob=(0.025, 0.25), grad_scale=0.025)
        self.whiten_keys = Whiten(num_groups=num_heads, whitening_limit=2.0, prob=(0.025, 0.25), grad_scale=0.025)
        self.linear_pos = ScaledLinear(
            embed_dim, num_heads * pos_dim, bias=False, initial_scale=0.05
        )
        self.copy_pos_query = Identity()
        self.copy_query = Identity()
        self.out_proj = ScaledLinear(attention_dim // 2, embed_dim, bias=True, initial_scale=0.05)
        self.in_proj2 = nn.Linear(embed_dim, attention_dim // 2, bias=False)
        self.out_proj2 = ScaledLinear(attention_dim // 2, embed_dim, bias=True, initial_scale=0.05)
        # self.whiten_values2 is applied on the values in forward2()
        self.whiten_values2 = Whiten(num_groups=num_heads, whitening_limit=2.0, prob=(0.025, 0.25), grad_scale=0.025)

    def forward(self, x: Tensor, pos_emb: Tensor, key_padding_mask: Optional[Tensor] = None, attn_mask: Optional[Tensor] = None):
    ...
    def multi_head_attention_forward(
        ...
        if attn_mask is not None:
            ...
            if attn_mask.dim() == 2:
                ...
                if list(attn_mask.size()) != [bsz * num_heads, seq_len, seq_len]:
        ...
        if key_padding_mask is not None:
            ...
            assert key_padding_mask.size(1) == seq_len, "{} == {}".format(key_padding_mask.size(1), seq_len)
        ...
        attn_output_weights = attn_output_weights.view(bsz * num_heads, seq_len, seq_len)
        if attn_mask is not None:
...
class DownsampledZipformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.out_combiner = SimpleCombiner(input_dim, output_dim, min_weight=(0.0, 0.25))
...
class AttentionDownsample(torch.nn.Module):
    def __init__(self, in_channels: int, out_channels: int, downsample: int):
        ...
        if out_channels > in_channels:
            self.extra_proj = nn.Linear(in_channels * downsample, out_channels - in_channels, bias=False)
        else:
...
class ConvolutionModule(nn.Module):
    def __init__(self, channels: int, kernel_size: int, bias: bool = True) -> None:
        ...
        self.deriv_balancer1 = ActivationBalancer(2 * channels, channel_dim=1, max_abs=10.0, min_positive=0.05, max_positive=1.0)
        self.depthwise_conv = nn.Conv1d(channels, channels, kernel_size, stride=1, padding=(kernel_size - 1) // 2, groups=channels, bias=bias)
        self.deriv_balancer2 = ActivationBalancer(channels, channel_dim=1, min_positive=0.05, max_positive=1.0, max_abs=20.0)
        self.activation = DoubleSwish()
        self.pointwise_conv2 = ScaledConv1d(channels, channels, kernel_size=1, stride=1, padding=0, bias=bias, initial_scale=0.05)
...
class RelPositionalEncoding(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tensor:
        self.extend_pe(x)
        pos_emb = self.pe[:, self.pe.size(1) // 2 - x.size(0) + 1 : self.pe.size(1) // 2 + x.size(0)]
