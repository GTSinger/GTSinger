...
from research.singtech.te_dataset2 import TEDataset2
...
class TETask2(SpeechBaseTask):
    def __init__(self, *args, **kwargs):
        super(SpeechBaseTask, self).__init__(*args, **kwargs)
        self.dataset_cls = TEDataset2
        self.vocoder = None
    ...
    def run_model(self, sample, infer=False):
        ...
        if not infer:
            ...
            if hparams.get('apply_tech_group_loss', False) and tech_ids is not None:
                ...
                self.add_tech_loss(output['tech_logits'], techs[:,:output['tech_logits'].shape[1],:], losses)
    ...
    def add_tech_group_loss(self, tech_logits, techs, tech_ids, losses):
        ...
        for b_idx in range(bsz):
            tech_losses_i = F.binary_cross_entropy_with_logits(tech_logits[b_idx, :, tech_ids[b_idx]], techs[b_idx, :tech_logits.shape[1], tech_ids[b_idx]], reduction='none')  # [T]
    ...
    def add_tech_loss(self, tech_logits, techs, losses):
        bsz, T, num_techs = tech_logits.shape
        tech_losses = F.binary_cross_entropy_with_logits(tech_logits, techs.float()[:,:T,:], reduction='none')  # [B, T, C]
        ...
        if batch_idx < hparams['num_valid_stats']:
            ...
            with torch.no_grad():
                ...
                if torch.sum(techs) > 0:
                    outputs['losses']['tech_auroc'] = binary_auroc(tech_logits, techs[:,:tech_logits.shape[1]:], threshold)
                    outputs['losses']['tech_p'] = binary_precision(tech_logits, techs[:,:tech_logits.shape[1]:], threshold)
                    outputs['losses']['tech_r'] = binary_recall(tech_logits, techs[:,:tech_logits.shape[1]:], threshold)
                    outputs['losses']['tech_f'] = binary_f1_score(tech_logits, techs[:,:tech_logits.shape[1]:], threshold)
                outputs['losses']['tech_a'] = binary_accuracy(tech_logits, techs[:,:tech_logits.shape[1]:], threshold)
                ...
                for tech_idx, tech_name in enumerate(tech_names):
                    if torch.sum(techs[:, :, tech_idx]) > 0:
                        outputs['losses'][f'{tech_name}_auroc'] = binary_auroc(tech_logits[:, :, tech_idx], techs[:, :tech_logits.shape[1], tech_idx], threshold)
                        outputs['losses'][f'{tech_name}_p'] = binary_precision(tech_logits[:, :, tech_idx], techs[:, :tech_logits.shape[1], tech_idx], threshold)
                        outputs['losses'][f'{tech_name}_r'] = binary_recall(tech_logits[:, :, tech_idx], techs[:, :tech_logits.shape[1], tech_idx], threshold)
                        outputs['losses'][f'{tech_name}_f'] = binary_f1_score(tech_logits[:, :, tech_idx], techs[:, :tech_logits.shape[1], tech_idx], threshold)
                    outputs['losses'][f'{tech_name}_a'] = binary_accuracy(tech_logits[:, :, tech_idx], techs[:, :tech_logits.shape[1], tech_idx], threshold)
    ...
    def validation_end(self, outputs):
        # torch.cuda.empty_cache()
        return super(TETask2, self).validation_end(outputs)
